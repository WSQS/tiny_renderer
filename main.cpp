...
public:
    ...
    {
        VaryingIntensity[nthvert] = std::max(0.f, model->GetNorm(iface, nthvert) * light_dir);
        VaryingU[nthvert] = static_cast<float>(model->uv(iface, nthvert)[0]);
        VaryingV[nthvert] = static_cast<float>(model->uv(iface, nthvert)[1]);
        // get diffuse lighting intensity
        Vec3f gl_Vertex{model->vert(iface, nthvert)}; // read the vertex from .obj file
        // TODO:待矩阵化
        gl_Vertex = Vec3f(BuildAxis() * Matrix(gl_Vertex - CenterOfScreen));
